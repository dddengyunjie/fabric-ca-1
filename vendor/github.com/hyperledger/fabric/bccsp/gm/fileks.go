...
func (ks *fileBasedKeyStore) GetKey(ski []byte) (k bccsp.Key, err error) {
	...
	case "pk":
		...
		fmt.Printf("[matrix] fileks.go 151, key: %v", key)

		switch key.(type) {
		case *ecdsa.PublicKey:
			fmt.Println("[matrix] fuck run here")
			ecdsaPublicKey := key.(*ecdsa.PublicKey)
			return &gmsm2PublicKey{&sm2.PublicKey{Curve: sm2.P256Sm2(), X: ecdsaPublicKey.X, Y: ecdsaPublicKey.Y}}, nil
		case *sm2.PublicKey:
...
func (ks *fileBasedKeyStore) storePrivateKey(alias string, privateKey interface{}) error {
	rawKey, err := x509.WritePrivateKeyToPem(privateKey.(*sm2.PrivateKey), nil)
	if err != nil {
